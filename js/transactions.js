    ...
    .map((t) => {
      ...
      if (status === "Pending") {
        if (isAdmin() || isOfficer()) {
          actionHtml = `<button class="btn btn-green btn-sm" onclick="approveBorrow(${t.id})">Approve</button>
                        <button class="btn btn-red btn-sm" onclick="rejectBorrow(${t.id})">Reject</button>`;
        } else if (isBorrower() && t.user_id === currentUserId) {
          actionHtml = `<button class="btn btn-outline btn-sm" onclick="cancelBorrow(${t.id})">Cancel</button>`;
        } else {
          actionHtml = `<span class="muted text-sm">Awaiting approval</span>`;
        }
      } else if (status === "Approved") {
        if (isAdmin() || isOfficer()) {
          actionHtml = `<button class="btn btn-green btn-sm" onclick="claimEquipment(${t.id})">Claim</button>`;
        } else {
          actionHtml = `<span class="muted text-sm">Approved</span>`;
        }
...
// ----- Cancel a pending borrow request (Borrower) -----
async function cancelBorrow(id) {
  if (!confirm("Are you sure you want to cancel your request?")) return;
  const { error } = await SUPABASE.rpc("cancel_borrow", { p_txn_id: id });
  if (error) {
...
// ----- Claim Equipment (Officer) -----
async function claimEquipment(id) {
  const txn = allTransactions.find((t) => t.id === id);
  const equip = equipmentMap.get(txn.equipment_id);
  if (!confirm(`Mark "${equip.equipment_name}" as Borrowed by ${txn.borrower_name}?`)) return;

  const { error } = await SUPABASE.rpc("claim_equipment", { p_txn_id: id });
  if (error) {
    showToast("Claim failed: " + (error.message || error.details), "error");
    return;
  }
  showToast("Equipment marked as Borrowed.");
  await loadData();
}
