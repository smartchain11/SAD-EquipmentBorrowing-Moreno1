...
function computedStatus(txn) {
  if (!["Returned", "Rejected", "Cancelled"].includes(txn.status) && txn.due_date && txn.due_date < today()) {
    return "Overdue";
  }
  return txn.status;
}
...
async function renderShell(active = "") {
  ...
  if (user) {
    const labels = { administrator: "Administrator", officer: "Officer", borrower: "Borrower" };
    document.getElementById("user-name").textContent = user.email + " (" + (labels[currentRole] || currentRole) + ")";
  }
  ...
  nav.innerHTML = links
    .map(([href, label]) => `<a href="${href}" class="${active === label ? "active" : ""}">${label}</a>`)
    .join("");
}