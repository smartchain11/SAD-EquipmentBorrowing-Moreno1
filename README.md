...
| `status`        | text      | Pending / Approved / Rejected / Borrowed / Returned / Overdue / Cancelled |
...
| --------------- | --------- | -------------------------------------- |
| `id`            | bigint    | Primary key                            |
| `action`        | text      | SUBMIT, APPROVE, COMPLETE, etc.        |
...
| `performed_at`  | timestamp | Time of action                         |
...
| BR-06 | New requests receive Pending status             | `record_borrow()`       |
| BR-07 | Approved requests reserve equipment availability| `claim_equipment()`         |
...
| BR-12 | Audit logs capture all system actions           | `audit_logs` + Triggers   |
...
│   ├── audit-logs.js     # Audit logging module
